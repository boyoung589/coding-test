...
//1차 => Error: Timeout of 2000ms exceeded. : 성능을 위해 제곱근 사용필요
function gcd(m,n){
    return m % n === 0 ? n : gcd(n, m % n);
};

function divideChocolateStick_1(M, N) {
    // M과 N의 공약수들을 구해야함 === 최대공약수의 약수
    let arr = [];
    let gcdMN = gcd(M, N);

    for(let i = 1; i <= gcdMN; i++){
        if(gcdMN % i === 0){
        arr.push([i, M/i, N/i])
        }
    }
...
function divideChocolateStick(M, N) {
    ...
    /*  1 ...gcdSqrt...gcdMN
        ...
        M=20, N=10일때 #주의: 최대공약수의 제곱근은 최대공약수의 약수가 아니다!!!
        1 ... 2...[3.xx]...5 ... 10
    */

    for(let i = 1; i <= gcdSqrt; i++){
        if(gcdMN % i === 0){
        arr.push([i, M/i, N/i])
        if(i*i < gcdMN){
            let num = gcdMN/i;
            arr.push([num, M/num, N/num])
        };
        };
    }
